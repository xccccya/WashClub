...
> 由于这个项目从头到现在都是靠AI一点一点摸索出来，本人只有最基础的电脑知识。README文档也大部分由AI生成实际内容可能和项目实际情况不符，请自行鉴别；但项目我本人已经在生产环境使用（超级小体量），所以可靠和稳定性都无法保证。同时也欢迎大佬帮助完善此项目，我深知全靠AI搓起来的这个项目安全性稳定性都无法保证。开发此项目起因是我想自制一个自家店铺用的会员营销小程序，任何功能都可以随自己想法。联系我请发邮件至chen_nanzhi#qq.com
...
### v1.3.0 正式版 (2025-01-04)
####后端
更新了项目依赖/Prisma/Nest版本


####小程序
更新了项目依赖版本
优化了大量页面UI/UX 使用体验up
增强洗车卡支付逻辑，新增他人卡主手机号后四位的记录与展示；优化订单支付后通知逻辑，确保卡主和下单人均能接收到相关信息；重构订单详情页面的时间线展示，提升用户体验。

####管理端
更新了项目依赖版本
优化了首页仪表盘UI/UX,数据筛选加入了上月/按月筛选的快速选项以及自定义时间区间筛选。加入了当前选择区间内每日详情的查看卡片。
修复了广告横幅无法添加/管理问题
修改了积分获取规则，不再使用实际支付1分对应多少积分而是改为更常见的1元对应多少积分
优化了订单详情页面UI/UX 使各个信息卡片之间更加统一，视觉不割裂。加入了订单积分变动情况查看的卡片。
优化了订单列表订单号列的操作逻辑和UI。
...
优化了订单详情页面UI/UX 使各个信息卡片之间更加统一，视觉不割裂。加入了订单积分变动情况查看的卡片。
优化了订单列表订单号列的操作逻辑和UI。
优化/修复了服务队列移除/添加等操作的一些问题。
### V1.2.0 正式版 (2025-12-31)

#### 后端
修复/优化/完善了大量遗留问题，优化了前端的API接口调用，不再使用大量硬编码，改为对接OPENAPI SDK,更加规范适应后续开发。

#### 小程序
优化
1.优化了登录页面UI/UX，更加美观易用。

### V1.1.0 正式版 (2025-10-09)

#### 收银端
- 优化：
新增“智能选择”为默认入口，车牌输入支持联想与车牌键盘，选择更顺手。
品牌/车系在慢网场景下显示骨架屏与空态提示，加载体验更平滑。
开单向导与模式切换时自动清理历史输入与选择，避免误用上一辆车数据。
智能选择默认“绑定到会员”，减少误操作，提升开单效率。
打开向导与提交后自动清空车辆及服务选择，连续创建更顺畅。
车牌联想与输入响应更稳定，选择与新建流程更清晰。
优化订单列表页面UI.UX，更加美观易用。
优化洗车卡结算功能，支持自定义选择扣卡会员/指定卡。
- 修复：
解决智能模式下车牌联想不自动弹出的情况。
解决车牌键盘弹层可能遮挡联想下拉的情况。
修复切换会员后车辆未同步清空的问题。
修复修改车牌后仍保留之前已选车辆的情况。
选择一次车牌后再次创建仍沿用上次车辆信息的问题。
智能选择中选过已有车牌后，清空或输入不存在车牌无法再次出现“新建车辆”提示的问题。
切换会员后未自动清除已选车辆，可能导致下单车辆错用的问题。
四种选车方式在成功入队后未清理导致的各种残留数据问题。
- 新增：
“智能选择”模式（默认）。
新建车辆卡片右上角帮助说明。
品牌/车系骨架屏与空态提示。
...
#### 小程序
优化
1.统计时间对齐到自然日，数据展示更准确、与页面一致
2.首页洗车建议的算法逻辑
3.支持用户直接绑定店铺已经注册添加的车辆车牌号。
修复
1.解决选择时间后多出“开始日前一天”或该天显示为 0 的问题
2.未选时间时默认展示“近 7 日”，区间更符合直觉
新增
1.店铺营业状态展示。
#### 管理后台
优化
1.统计时间对齐到自然日，数据展示更准确、与页面一致
2.优化订单列表页面UI.UX，更加美观易用。
3.优化洗车卡结算功能，支持自定义选择扣卡会员/指定卡。
修复
1.解决选择时间后多出“开始日前一天”或该天显示为 0 的问题
2.未选时间时默认展示“近 7 日”，区间更符合直觉
新增
...
巨科汽车美容管理系统是一个功能完整的汽车服务行业门店管理平台，包含会员管理、订单处理、支付系统、库存管理、售后服务等核心功能。
...
本项目采用 **Apache License 2.0** 开源，详情见 [`LICENSE`](./LICENSE)。

Copyright (c) 2025 Yuki



**巨科汽车美容管理系统** - 让汽服行业门店管理更智能、更高效！ 🚗✨
