...
> 文档状态：2026-08-17 已按本文完成代码、migration、OpenAPI/SDK、管理后台和小程序实现；migration 尚未应用，真实高德/微信支付/退款、后台定位与真机流程仍须在目标环境验收。
>
...
| 支付结算 | 先支付预估车费；最终费用低于预付则部分退款，高于预付则创建补款订单 |
...
过路费由高德路线结果提供预估值，停车费和其他费用由司机在费用确认页面添加，并根据配置开关决定是否允许。
...
- 最终费用小于预付金额：对主订单执行部分退款，记录退款流水和时间线。
- 最终费用等于预付金额：直接完成。
- 最终费用大于预付金额：创建行程补款订单，复用现有微信 JSAPI 支付；补款成功后完成。
...
- 行程订单详情使用高德 JSAPI 绘制订单地图，显示起点、终点、服务端规划路线和实际行驶轨迹，并展示关键状态时间点、费用流水、补款、退款和异常状态。
...
- 司机、起点、终点使用 Marker；规划路线和实际轨迹使用不同颜色的 Polyline，并提供图例。
