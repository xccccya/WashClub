<template>
	<view class="page">
		...
		<view class="card">
			<view class="header"><text class="title">编辑收货地址</text></view>
			<view class="form">
				<view class="form-row">
...
function goBack(){
    try { uni.navigateBack(); } catch { uni.reLaunch({ url: '/pages/address/index' }); }
}
...
async function initCascade(){
    await loadProvinces();
    if (form.province){ const p = provinceList.value.find(i=>i.name===form.province); if (p){ cityList.value = await fetchSub(p.adcode || p.name); } }
    if (form.city){ const c = cityList.value.find(i=>i.name===form.city); if (c){ districtList.value = await fetchSub(c.adcode || c.name); } }
    if (form.district){ const d = districtList.value.find(i=>i.name===form.district); if (d){ streetList.value = await fetchSub(d.adcode || d.name); } }
}
