		...
		<view class="list">
			<view v-for="item in visibleItems" :key="item.id" class="trip-card" @tap="open(item.id)">
				<view class="card-head">
					<view class="status" :class="statusTone(item.status)"><text class="status-dot" />{{ statusText(item.status) }}</view>
				...
				<view class="card-foot">
					<view class="driver"><image src="/static/icons/ride-vehicle-location.svg" mode="aspectFit" /><text>{{ item.driverEmployee?.name || item.driverMember?.name || '等待匹配司机' }}</text></view>
					<view class="amount"><text>行程金额</text><strong>¥{{ money(item.finalAmount ?? item.estimatedAmount) }}</strong><text class="chevron">›</text></view>
				</view>
			</view>
			<view v-if="!loading && !visibleItems.length" class="empty"><image src="/static/icons/ride-vehicle-location.svg" mode="aspectFit" /><strong>暂无相关行程</strong><text>{{ filter === 'all' ? '完成首次呼叫后，记录会出现在这里' : '换个状态看看其他行程' }}</text></view>
...
const activeStatuses = ['PREPAY_PENDING','DISPATCHING','TO_PICKUP','ARRIVED_PICKUP','IN_TRIP','ARRIVED_DESTINATION','FARE_PENDING','SUPPLEMENT_PENDING','REFUND_PENDING'];
...
const visibleItems = computed(() => items.value.filter((item) => {
	...
	if (filter.value === 'cancelled') return ['CANCELLED','NO_DRIVER'].includes(item.status);
	return true;
}));
...
function statusText(value:string){return({PREPAY_PENDING:'待预付',DISPATCHING:'等待司机接单',TO_PICKUP:'司机前往上车点',ARRIVED_PICKUP:'司机已到达',IN_TRIP:'行程中',ARRIVED_DESTINATION:'已到目的地',FARE_PENDING:'费用确认',SUPPLEMENT_PENDING:'待补款',REFUND_PENDING:'退款中',COMPLETED:'已完成',CANCELLED:'已取消',NO_DRIVER:'暂无司机'}as any)[value]||value}
