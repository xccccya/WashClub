<template>
	<view class="page">
		...
		<view v-if="offlineConfirmVisible" class="confirm-mask" @tap.stop>
			<view class="confirm-card" @tap.stop>
				<strong>确认已收到线下支付？</strong>
				<text>请确认乘客已通过现金或转账付清最终费用与线上预付的差额。确认后订单将直接完成，不再向乘客发起线上补款。</text>
				<view class="confirm-actions">
					<button class="confirm-cancel" :disabled="loading" @tap="closeOfflineConfirm">再检查一下</button>
					<button class="confirm-submit" :loading="loading" :disabled="loading" @tap="confirmOffline">确认已收款</button>
				</view>
			</view>
		</view>
	</view>
