		...
		<view v-if="trip" class="drawer-shell">
			...
			<view class="drawer-content">
				...
				<view v-if="fareDetails" class="fare-card">
					...
					<view class="settlement-grid"><view><text>{{ fareDetails.customPrepayEnabled ? '线上预付' : '预付' }}</text><strong>¥{{ money(fareDetails.prepaidAmount) }}</strong></view><view v-if="Number(fareDetails.offlinePaidAmount || 0)"><text>已确认线下支付</text><strong class="success">¥{{ money(fareDetails.offlinePaidAmount) }}</strong><text v-if="trip.offlinePaidAt" class="settlement-time">确认时间：{{ formatTime(trip.offlinePaidAt) }}</text></view><view v-if="Number(fareDetails.supplementAmount || 0)"><text>需补款</text><strong class="warn">¥{{ money(fareDetails.supplementAmount) }}</strong></view><view v-if="Number(fareDetails.refundableAmount || 0)"><text>应退差额</text><strong class="success">¥{{ money(fareDetails.refundableAmount) }}</strong></view><view v-if="Number(fareDetails.refundedAmount || 0)"><text>已退款</text><strong class="success">¥{{ money(fareDetails.refundedAmount) }}</strong></view></view>
			</view>
...
const statusHint = computed(() => { if (trip.value?.status === 'REFUND_PENDING' && refundCompleted.value) return trip.value?.finalAmount == null ? '本次行程退款已按原渠道完成' : '最终费用已结算，预付差额已退回'; return ({ PREPAY_PENDING: '完成预付后开始派单', DISPATCHING: isDriverView.value ? '可查看乘客与路线信息后接单' : '正在通知附近空闲司机', TO_PICKUP: isDriverView.value ? '请按导航前往上车点' : '请留意司机实时位置', ARRIVED_PICKUP: isDriverView.value ? '请核验乘客手机号后四位' : '请与司机确认后上车', IN_TRIP: '费用与司机端计价器实时同步', ARRIVED_DESTINATION: '司机确认最终费用后，可线上支付差额；司机也可确认已线下收款', FARE_PENDING: '费用确认后将完成行程', SUPPLEMENT_PENDING: '请完成实际费用高于预付金额的线上差额支付', REFUND_PENDING: '退款已提交，将按原支付渠道退回', COMPLETED: '感谢使用内部用车服务', CANCELLED: Number(trip.value?.order?.refundedAmount || 0) > 0 ? '本次行程已取消，退款已完成' : '本次行程已结束', NO_DRIVER: Number(trip.value?.order?.refundedAmount || 0) > 0 ? '未匹配到司机，退款已完成' : '可稍后重试呼叫' } as Record<string, string>)[trip.value?.status] || '行程状态将实时更新'; });
...
.settlement-grid .settlement-time{margin-top:4rpx;color:#94a3b8;font-size:14rpx;line-height:1.35}
</style>
