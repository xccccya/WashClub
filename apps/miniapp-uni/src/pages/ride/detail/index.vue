<template>
	<view class="page">
		...
		<scroll-view v-if="trip" scroll-y class="drawer">
			<view class="drawer-content">
				...
				<view class="driver-card">
					...
					<button v-if="canContact" size="mini" class="contact" @tap="callOtherParty">电话</button><button v-if="canContact" size="mini" class="contact" @tap="chat = true">消息</button>
				</view>
				<view v-if="fareDetails" class="fare-card">
					...
					<view class="fare-grid">
						<view><text>起步价</text><strong>¥{{ money(fareDetails.baseFare) }}</strong></view>
						<view><text>里程费</text><strong>¥{{ money(fareDetails.distanceFare) }}</strong><small>超出 {{ numberText(fareDetails.chargeableDistanceKm) }}km × ¥{{ money(fareDetails.pricePerKm) }}</small></view>
						<view><text>时长费</text><strong>¥{{ money(fareDetails.durationFare) }}</strong><small>超出 {{ numberText(fareDetails.chargeableDurationMinutes) }}分钟 × ¥{{ money(fareDetails.pricePerMinute) }}</small></view>
						<view><text>过路费</text><strong>¥{{ money(fareDetails.tollAmount) }}</strong></view>
						<view v-if="Number(fareDetails.parkingAmount || 0)"><text>停车费</text><strong>¥{{ money(fareDetails.parkingAmount) }}</strong></view><view v-if="Number(fareDetails.otherAmount || 0)"><text>其他费用</text><strong>¥{{ money(fareDetails.otherAmount) }}</strong></view>
					</view>
					<view class="fare-rule">含 {{ numberText(fareDetails.includedDistanceKm) }}km / {{ numberText(fareDetails.includedDurationMinutes) }}分钟{{ fareDetails.minimumApplied ? ` · 最低消费 ¥${money(fareDetails.minimumFare)}` : '' }}</view>
		...
		<view v-else class="loading">正在加载行程…</view><RideChatSheet v-if="trip" v-model="chat" :ride-id="trip.id" :member-id="memberId" />
	</view>
</template>
...
const trip = ref<any>(null); const chat = ref(false); const vehicleLogoFailed = ref(false); let id = 0; const isDriverView = ref(false); let stopRealtime: (() => void) | null = null; let poll: ReturnType<typeof setInterval> | undefined;
...
const markers = computed(() => { if (!trip.value) return []; const current = trip.value; const list: any[] = [{ id: 1, longitude: Number(current.originLongitude), latitude: Number(current.originLatitude), title: '起点', kind: 'origin' }, { id: 2, longitude: Number(current.destinationLongitude), latitude: Number(current.destinationLatitude), title: '终点', kind: 'destination' }]; const location = current.locations?.[0]; if (location) list.push({ id: 3, longitude: Number(location.longitude), latitude: Number(location.latitude), title: '司机位置', kind: 'driver-current' }); return list; });
...
const passengerLabel = computed(() => `乘客${trip.value?.passenger?.phoneLastFour || ''}`);
...
function handleRealtime(event: any) { if (event?.type === 'ride:meter' && Number(event?.data?.rideTripId) === id && trip.value) { trip.value.fareDetails = { ...(trip.value.fareDetails || {}), ...(event.data.meter || {}), mode: 'LIVE' }; return; } void load(); }
...
<style scoped>
.page{height:100vh;position:relative;overflow:hidden;background:#e2e8f0}.map{height:100%}.drawer{position:absolute;z-index:20;right:20rpx;bottom:calc(24rpx + env(safe-area-inset-bottom));left:20rpx;max-height:66vh;border:1rpx solid rgba(255,255,255,.8);border-radius:34rpx;background:rgba(255,255,255,.97);box-shadow:0 18rpx 52rpx rgba(15,23,42,.2);box-sizing:border-box}.drawer-content{padding:24rpx}.status-line{display:flex;align-items:center;gap:16rpx;margin-bottom:18rpx}.status-line view:last-child{min-width:0}.status-line strong,.status-line text{display:block}.status-line strong{color:#0f172a;font-size:30rpx}.status-line text{margin-top:4rpx;color:#64748b;font-size:22rpx}.status-dot{width:18rpx;height:18rpx;border:8rpx solid #dbeafe;border-radius:50%;background:#3580ff}.route-card{padding:18rpx;border-radius:22rpx;background:#f8fafc}.route-row{display:flex;align-items:center;gap:14rpx;color:#334155;font-size:25rpx}.route-row>text:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.point{display:grid;flex:none;width:40rpx;height:40rpx;place-items:center;border-radius:50%;color:#fff;font-size:20rpx}.origin{background:#22c55e}.destination{background:#f43f5e}.route-link{width:2rpx;height:22rpx;margin:4rpx 0 4rpx 19rpx;background:#cbd5e1}.driver-card{display:flex;align-items:center;gap:12rpx;margin-top:18rpx;padding:16rpx;border:1rpx solid #dbeafe;border-radius:22rpx;background:#eff6ff}.driver-card image{width:66rpx;height:66rpx;flex:none;border-radius:16rpx;background:#fff}.driver-copy{min-width:0;flex:1}.driver-copy strong,.driver-copy text{display:block}.driver-copy strong{color:#0f172a;font-size:27rpx}.driver-copy text{margin-top:4rpx;overflow:hidden;color:#64748b;font-size:22rpx;text-overflow:ellipsis;white-space:nowrap}.contact{flex:none;margin:0;padding:0 16rpx;border:0;background:#fff;color:#2563eb;font-size:22rpx}.fare-card{margin-top:18rpx;padding:20rpx;border-radius:24rpx;background:linear-gradient(145deg,#0f172a,#1e293b);box-shadow:0 12rpx 30rpx rgba(15,23,42,.2);color:#fff}.fare-head,.settlement-grid{display:flex;align-items:center;justify-content:space-between}.fare-head>view:first-child text,.fare-head>view:first-child strong{display:block}.fare-head>view:first-child text{font-size:21rpx;opacity:.68}.fare-head>view:first-child strong{margin-top:2rpx;font-size:48rpx}.fare-trip{display:flex;gap:10rpx}.fare-trip text{padding:8rpx 12rpx;border-radius:999rpx;background:rgba(255,255,255,.1);font-size:19rpx}.fare-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10rpx;margin-top:16rpx}.fare-grid>view{padding:13rpx 14rpx;border-radius:16rpx;background:rgba(255,255,255,.08)}.fare-grid text,.fare-grid strong,.fare-grid small{display:block}.fare-grid text{font-size:19rpx;opacity:.65}.fare-grid strong{margin-top:3rpx;font-size:26rpx}.fare-grid small{margin-top:3rpx;font-size:16rpx;opacity:.55}.fare-rule{margin-top:13rpx;color:#cbd5e1;font-size:18rpx}.settlement-grid{flex-wrap:wrap;gap:12rpx;margin-top:16rpx;padding-top:14rpx;border-top:1rpx solid rgba(255,255,255,.12)}.settlement-grid>view{min-width:42%}.settlement-grid text,.settlement-grid strong{display:block}.settlement-grid text{font-size:18rpx;opacity:.62}.settlement-grid strong{margin-top:2rpx;font-size:24rpx}.settlement-grid .warn{color:#fbbf24}.settlement-grid .success{color:#6ee7b7}.refund-note{display:flex;align-items:center;justify-content:space-between;margin-top:16rpx;padding:16rpx 18rpx;border-radius:18rpx;background:#fff7ed;color:#9a3412;font-size:22rpx}.refund-note strong{font-size:24rpx}.refund-note text{color:#c2410c}.primary,.cancel{margin-top:16rpx;border:0;border-radius:44rpx}.primary{background:#0f172a;color:#fff}.cancel{background:#f8fafc;color:#64748b}.loading{position:absolute;top:45%;right:80rpx;left:80rpx;z-index:20;padding:24rpx;border-radius:24rpx;background:rgba(255,255,255,.92);color:#64748b;text-align:center}button::after{border:0}
</style>
