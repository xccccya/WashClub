		...
		<scroll-view v-if="trip" scroll-y class="drawer">
			<view class="drawer-content">
				...
				<view class="driver-card">
					...
					<button v-if="canContact" size="mini" class="contact" @tap="callOtherParty">电话</button><button v-if="canContact" size="mini" class="contact message-contact" @tap="chat = true"><text>消息</text><text v-if="chatUnreadCount" class="unread-badge">{{ chatUnreadCount > 99 ? '99+' : chatUnreadCount }}</text></button>
				</view>
...
const trip = ref<any>(null); const chat = ref(false); const chatUnreadCount = ref(0); const vehicleLogoFailed = ref(false); const fareExpanded = ref(false); let id = 0; const isDriverView = ref(false); let stopRealtime: (() => void) | null = null; let poll: ReturnType<typeof setInterval> | undefined;
...
<style scoped>
.page{height:100vh;position:relative;overflow:hidden;background:#e2e8f0}.map{height:100%}.drawer{position:absolute;z-index:20;right:20rpx;bottom:calc(24rpx + env(safe-area-inset-bottom));left:20rpx;max-height:70vh;border:1rpx solid rgba(255,255,255,.8);border-radius:34rpx;background:rgba(255,255,255,.97);box-shadow:0 18rpx 52rpx rgba(15,23,42,.2);box-sizing:border-box}.drawer-content{min-width:0;padding:24rpx;box-sizing:border-box}.status-line{display:flex;align-items:center;gap:16rpx;margin-bottom:18rpx}.status-line view:last-child{min-width:0}.status-line strong,.status-line text{display:block}.status-line strong{color:#0f172a;font-size:30rpx}.status-line text{margin-top:4rpx;color:#64748b;font-size:22rpx}.status-dot{width:18rpx;height:18rpx;border:8rpx solid #dbeafe;border-radius:50%;background:#3580ff}.route-card{padding:18rpx;border-radius:22rpx;background:#f8fafc}.route-row{display:flex;align-items:center;gap:14rpx;color:#334155;font-size:25rpx}.route-row>text:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.point{display:grid;flex:none;width:40rpx;height:40rpx;place-items:center;border-radius:50%;color:#fff;font-size:20rpx}.origin{background:#22c55e}.destination{background:#f43f5e}.route-link{width:2rpx;height:22rpx;margin:4rpx 0 4rpx 19rpx;background:#cbd5e1}.driver-card{display:flex;align-items:center;gap:12rpx;margin-top:18rpx;padding:16rpx;border:1rpx solid #dbeafe;border-radius:22rpx;background:#eff6ff}.driver-card image{width:66rpx;height:66rpx;flex:none;border-radius:16rpx;background:#fff}.driver-copy{min-width:0;flex:1}.driver-copy strong,.driver-copy text{display:block}.driver-copy strong{color:#0f172a;font-size:27rpx}.driver-copy text{margin-top:4rpx;overflow:hidden;color:#64748b;font-size:22rpx;text-overflow:ellipsis;white-space:nowrap}.contact{flex:none;margin:0;padding:0 14rpx;border:0;background:#fff;color:#2563eb;font-size:21rpx}.message-contact{position:relative}.unread-badge{position:absolute;top:-12rpx;right:-10rpx;display:grid;min-width:30rpx;height:30rpx;padding:0 7rpx;place-items:center;border:3rpx solid #eff6ff;border-radius:999rpx;background:#ef4444;color:#fff;font-size:16rpx;font-weight:800;line-height:1;box-sizing:border-box}.fare-card{width:100%;min-width:0;margin-top:18rpx;padding:18rpx;border-radius:24rpx;background:linear-gradient(145deg,#0f172a,#1e293b);box-shadow:0 12rpx 30rpx rgba(15,23,42,.2);color:#fff;box-sizing:border-box;overflow:hidden}.fare-head,.settlement-grid,.fare-rule{display:flex;align-items:center;justify-content:space-between}.fare-head{gap:16rpx}.fare-head>view:first-child{min-width:0}.fare-head>view:first-child text,.fare-head>view:first-child strong{display:block}.fare-head>view:first-child text{font-size:19rpx;opacity:.68}.fare-head>view:first-child strong{margin-top:2rpx;font-size:43rpx;line-height:1.12}.fare-trip{display:flex;flex:none;gap:8rpx}.fare-trip text{padding:7rpx 10rpx;border-radius:999rpx;background:rgba(255,255,255,.1);font-size:17rpx;white-space:nowrap}.fare-breakdown{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7rpx;margin-top:14rpx}.fare-breakdown>view{min-width:0;padding:11rpx 6rpx;border-radius:14rpx;background:rgba(255,255,255,.08);text-align:center}.fare-breakdown text,.fare-breakdown strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.fare-breakdown text{font-size:16rpx;opacity:.62}.fare-breakdown strong{margin-top:3rpx;font-size:20rpx}.fare-details{display:grid;gap:8rpx;margin-top:10rpx;padding:12rpx 14rpx;border-radius:15rpx;background:rgba(255,255,255,.06)}.fare-details>view{display:flex;min-width:0;align-items:flex-start;justify-content:space-between;gap:12rpx}.fare-details text{flex:none;color:#94a3b8;font-size:17rpx}.fare-details strong{min-width:0;color:#e2e8f0;font-size:18rpx;font-weight:600;text-align:right;overflow-wrap:anywhere}.fare-rule{gap:12rpx;margin-top:12rpx;color:#cbd5e1;font-size:17rpx}.fare-rule>text:first-child{min-width:0;line-height:1.45}.fare-toggle{flex:none;color:#93c5fd;font-weight:700;white-space:nowrap}.settlement-grid{flex-wrap:wrap;gap:10rpx;margin-top:12rpx;padding-top:12rpx;border-top:1rpx solid rgba(255,255,255,.12)}.settlement-grid>view{min-width:0;flex:1 1 42%;padding:8rpx 10rpx;border-radius:12rpx;background:rgba(255,255,255,.05);box-sizing:border-box}.settlement-grid text,.settlement-grid strong{display:block}.settlement-grid text{font-size:16rpx;opacity:.62}.settlement-grid strong{margin-top:2rpx;font-size:21rpx}.settlement-grid .warn{color:#fbbf24}.settlement-grid .success{color:#6ee7b7}.refund-note{display:flex;align-items:center;justify-content:space-between;margin-top:16rpx;padding:16rpx 18rpx;border-radius:18rpx;background:#fff7ed;color:#9a3412;font-size:22rpx}.refund-note strong{font-size:24rpx}.refund-note text{color:#c2410c}.primary,.cancel{margin-top:16rpx;border:0;border-radius:44rpx}.primary{background:#0f172a;color:#fff}.cancel{background:#f8fafc;color:#64748b}.loading{position:absolute;top:45%;right:80rpx;left:80rpx;z-index:20;padding:24rpx;border-radius:24rpx;background:rgba(255,255,255,.92);color:#64748b;text-align:center}button::after{border:0}
</style>
