<template>
	<view class="page">
		...
		<view v-if="activeTrip" class="order-card">
			...
			<view class="passenger-row"><image class="avatar" :src="passengerAvatar" mode="aspectFill" /><view><strong>{{ passengerLabel }}</strong><text>仅用于本次行程联系</text></view><button @tap="callPassenger">电话</button><button @tap="chat = true">消息</button></view>
...
const passengerLabel = computed(() => '乘客' + (activeTrip.value?.passenger?.phoneLastFour || ''));
