<template>
	<view class="page">
		<RideMap :markers="markers" :route-points="routePoints" :fit-padding="mapFitPadding" locate-top="390rpx" />
		...
		<view v-if="activeTrip" class="order-card">
			...
			<view v-if="!orderCollapsed">
			<view class="passenger-row"><image class="avatar" :src="passengerAvatar" mode="aspectFill" /><view><strong>{{ passengerLabel }}</strong><text>仅用于本次行程联系</text></view><button @tap="callPassenger">电话</button><button class="message-button" @tap="chat = true"><text>消息</text><text v-if="chatUnreadCount" class="unread-badge">{{ chatUnreadCount > 99 ? '99+' : chatUnreadCount }}</text></button></view>
			...
			<template v-else-if="activeTrip.status === 'IN_TRIP'">
				<view class="meter"><view class="meter-head"><view><text>实时计价</text><strong>¥{{ money(meter?.amount) }}</strong></view><view><text>{{ distanceText(meter?.distanceMeters) }}</text><text>{{ durationText(meter?.durationSeconds) }}</text></view></view><view class="meter-breakdown"><view><text>起步价</text><strong>¥{{ money(meter?.baseFare) }}</strong></view><view><text>里程费</text><strong>¥{{ money(meter?.distanceFare) }}</strong></view><view><text>时长费</text><strong>¥{{ money(meter?.durationFare) }}</strong></view><view><text>附加费</text><strong>¥{{ money(meter?.extraAmount) }}</strong></view></view></view>
