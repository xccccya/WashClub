...
function onTypeMainChange(e:any){ const i = Number(e?.detail?.value||0); typeMain.value = typeMainOptions[i] || ''; }
function onTypeSubChange(e:any){ const i = Number(e?.detail?.value||0); typeSub.value = typeSubOptions(typeMain.value)[i] || ''; }
function onColorChange(e:any){ const i = Number(e?.detail?.value||0); color.value = colorOptions[i] || ''; }
...
function openSeriesDialog(){ if (!brandId.value) { uni.showToast({ title:'请先选择品牌', icon:'none' }); return; } seriesDialog.value = true; if (brandId.value) fetchSeriesIfNeeded(brandId.value); }
...
async function loadDetail(){
	...
	if (item) {
		...
		brandName.value = item.brand || '';
		seriesName.value = item.series || '';
		typeMain.value = item.typeMain || '';
		typeSub.value = item.typeSub || '';
...
onLoad((q:any)=>{ id.value = Number(q?.id || 0); });
onShow(async ()=>{ await safeCheckAuthAndRefresh({ redirectIfExpired: true }); if (id.value) loadDetail(); });
