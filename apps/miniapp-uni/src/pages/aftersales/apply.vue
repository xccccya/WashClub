	...
	try{
		...
		for (const filePath of r.tempFilePaths){
			await new Promise<void>((resolve)=>{
				uni.uploadFile({
					url: (http as any).baseUrl + '/file/upload',
					filePath,
					name: 'file',
					formData: { dir: 'miniapp' },
					header: { Authorization: 'Bearer ' + (http as any).getToken?.() },
					success: (res:any)=>{ try{ const j=JSON.parse(res.data||'{}'); if (j?.url) images.value.push(j.url); }catch{} resolve(); },
					fail: ()=> resolve(),
				});
			});
		}
