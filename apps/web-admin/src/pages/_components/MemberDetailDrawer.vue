					...
					<el-card shadow="never" class="tab-card">
						...
						<el-table-column prop="no" label="订单号" min-width="220">
							<template #default="{ row }">
								<el-link type="primary" :underline="false" @click="goOrder(row)">{{ row.no }}</el-link>
							</template>
						</el-table-column>
				...
				<div v-show="activeTab==='points'" class="tab-panel">
					<el-card shadow="never" class="tab-card">
						...
						<el-table-column label="关联订单" min-width="160">
							<template #default="{ row }">
								<el-link v-if="row.orderNo" type="primary" :underline="false" @click="goOrder(row)">{{ row.orderNo }}</el-link>
								<span v-else class="muted">—</span>
							</template>
						</el-table-column>
					...
					<el-card shadow="never" class="tab-card">
						<template #header>
							<div class="tab-card__header">
								...
								<div class="tab-card__actions">
									<el-button size="small" text @click="openMemberWashCardsManage" title="打开洗车计次卡管理并自动筛选该会员">
										去管理页
									</el-button>
									<el-button size="small" text @click="refreshActiveTab" title="刷新当前标签数据">
										<el-icon><Refresh /></el-icon>
					...
					<el-card shadow="never" class="tab-card">
						<template #header>
							<div class="tab-card__header">
								...
								<div class="tab-card__actions">
									<el-button size="small" text @click="openMemberVehiclesManage" title="打开车辆管理并自动筛选该会员">
										去管理页
									</el-button>
									<el-button size="small" text @click="refreshActiveTab" title="刷新当前标签数据">
										<el-icon><Refresh /></el-icon>
...
function goOrder(row: any) {
	try {
		...
		if (no) {
			window.open(`/admin/orders/no/${encodeURIComponent(no)}`, '_blank');
			return;
		}
	} catch {}
}
...
function safeMemberId(): number | null {
	const id = Number(props.memberId || member.value?.id || props.baseMember?.id || 0);
	return Number.isFinite(id) && id > 0 ? id : null;
}

function openMemberVehiclesManage() {
	const id = safeMemberId();
	if (!id) return;
	const url = `/admin/member-vehicles?memberId=${encodeURIComponent(String(id))}`;
	try { window.open(url, '_blank'); } catch { location.href = url; }
}

function openMemberWashCardsManage() {
	const id = safeMemberId();
	if (!id) return;
	const url = `/admin/member-washcards?memberId=${encodeURIComponent(String(id))}`;
	try { window.open(url, '_blank'); } catch { location.href = url; }
}
...
.drawer-body{
	...
	background: linear-gradient(
		180deg,
		var(--el-bg-color) 0%,
		color-mix(in oklab, var(--el-bg-color), var(--el-fill-color-light) 40%) 100%
	);
}
.header{
	display:flex;
...
.sep{ margin: 0 8px; color: var(--el-text-color-placeholder); }
...
.last-visit__label{ font-size: 12px; color: var(--el-text-color-secondary); }
.last-visit__value{ font-size: 12px; font-weight: 700; color: var(--el-text-color-primary); font-variant-numeric: tabular-nums; }
...
.tabs-inner :deep(.el-tabs__nav-wrap::after){ height: 1px; background: var(--el-border-color-lighter); }
...
.tab-card :deep(.el-card__header){
	...
	background: linear-gradient(
		180deg,
		color-mix(in oklab, var(--el-bg-color), transparent 0%) 0%,
		color-mix(in oklab, var(--el-bg-color), var(--el-fill-color-light) 40%) 100%
	);
}
...
.nice-table :deep(.el-table__row:hover td){
	background: color-mix(in oklab, var(--el-color-primary), transparent 94%) !important;
}
...
.section-title{ font-weight: 700; color: var(--el-text-color-primary); }
...
.mini-value{ font-size: 20px; font-weight: 900; color: var(--el-text-color-primary); font-variant-numeric: tabular-nums; }
.mini-label{ font-size: 12px; color: var(--el-text-color-secondary); margin-top: 2px; }
.divider{ width: 1px; align-self: stretch; background: var(--el-border-color-lighter); }
...
.section-title-marker{ width: 3px; height: 16px; border-radius: 2px; background: var(--el-color-primary); }
.section-title{ font-weight: 800; color: var(--el-text-color-primary); }
.kv-grid{
	display:grid;
...
.k{ color: var(--el-text-color-secondary); flex: 0 0 auto; }
.v{ color: var(--el-text-color-primary); min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
